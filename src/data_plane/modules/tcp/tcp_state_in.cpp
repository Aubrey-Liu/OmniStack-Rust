...
namespace omnistack::data_plane::tcp_state_in {
    ...
    inline void TcpStateIn::EnterSynReceived(TcpListenFlow* listen_flow, TcpFlow* flow, TcpHeader* tcp_header, uint16_t remote_mss, uint8_t remote_wscale, uint32_t remote_timestamp) {
        ...
        if(flow->state_ == TcpFlow::State::kListen) {
            ...
            std::string congestion_control_algorithm;
            if(listen_flow->congestion_control_algorithm_ != nullptr) congestion_control_algorithm = listen_flow->congestion_control_algorithm_;
            else congestion_control_algorithm = kTcpDefaultCongestionControlAlgorithm;
        ...
        if(tcp_header->syn) [[unlikely]] {
            ...
            if(!tcp_header->ack) {
                ...
                if(flow->state_ == TcpFlow::State::kEstablished) {
                    ...
                    raise_event_(new(event_pool_->Get()) TcpEventPassiveClose(local_ip, remote_ip, local_port, remote_port));
                }
