...
int main(int argc, char **argv) {
    ...
    app.add_option("--lhost", args::local_host, "hostname of client")->default_val("");
    app.add_option("--rhost", args::remote_host, "hostname of server")->default_val("");
    app.add_option("--lport", args::local_port, "Port of server")->default_val(23233);
    app.add_option("--rport", args::local_port, "Port of server")->default_val(23234);
    ...
    if (args::remote_host == "" && args::local_host == "") {
        OMNI_LOG(common::kError) << "No local hostname or remote hostname provided\n";
        return 1;
    }
    InitOmniStack();
    ...
    if (args::local_host != "") {
        ...
        local.sin_addr.s_addr = inet_addr(args::local_host.c_str());
        if (socket::bsd::bind(socket, (struct sockaddr *)&local, sizeof(local)) < 0) {
        ...
        if (args::is_udp) {
            ...
            if (args::is_pingpong) {
                ...
                if (!args::is_reversed) {
                    ...
                    while (true) {
                        auto new_packet = socket::fast::write_begin(client);
                        if(new_packet == nullptr) continue;
                        new_packet->SetLength(args::size);
                        int send_len = socket::fast::write(client, new_packet);
                        if (send_len < 0) {
                            OMNI_LOG(common::kError) << "Failed to send.\n";
